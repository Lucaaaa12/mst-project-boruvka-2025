...
public class QuickUnion<D> implements UnionFind<D> {
	...
	public void union(UnionFindNode<D> node1, UnionFindNode<D> node2) {
		if (node1.isRepresentative()==true && node2.isRepresentative()==true) {	//node1 e node2 sono rappresentanti differenti fra di loro
			if(find(node1)!=find(node2) ){
				((QuickUnionNode<D>) node2).parent=(QuickUnionNode<D>) node1; //rende il secondo nodo figlio del primo
			}
		}
	}
	...
	public UnionFindNode<D> find(UnionFindNode<D> node) {
		while(node.isRepresentative()==false){ //(node non è un rappresentante)
		node = ((QuickUnionNode<D>)node).parent; //passa al nodo padre	
		}
		return node;
	}
