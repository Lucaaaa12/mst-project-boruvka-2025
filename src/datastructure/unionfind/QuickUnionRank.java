	...
	public void union(UnionFindNode<D> node1, UnionFindNode<D> node2) {		
		if (node1.isRepresentative()==true && node2.isRepresentative()==true){		//node1 e node2 sono rappresentanti differenti fra di loro
			...
			if(find(node1)!=find(node2)){
				if (((QuickUnionRankNode<D>) node1).rank >= ((QuickUnionRankNode<D>) node2).rank){

					super.union(node1,node2); //rende node2 figlio di node1
					if (((QuickUnionRankNode<D>) node1).rank == ((QuickUnionRankNode<D>) node2).rank){ 
						((QuickUnionRankNode<D>) node1).rank ++;	//controlla se il rango aumenta
					}
				}
				//caso secondo albero > primo albero	
				else{
