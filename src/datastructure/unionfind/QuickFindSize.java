...
public class QuickFindSize<D> extends QuickFind<D> {
	...
	public void union(UnionFindNode<D> node1, UnionFindNode<D> node2) {
		if (find(node1) != find(node2)){
			if (((QuickFindSizeNode<D>)node1).size >= ((QuickFindSizeNode<D>)node2).size){
				super.union(node1,node2); //cambia la root a partire da node2
				((QuickFindSizeNode<D>)node1).size = ((QuickFindSizeNode<D>)node1).size+((QuickFindSizeNode<D>)node2).size;
			} //aggiorna il size
			else{
				super.union(node2,node1); //cambia la root a partire da node1
				((QuickFindSizeNode<D>)node2).size = ((QuickFindSizeNode<D>)node1).size+((QuickFindSizeNode<D>)node2).size; //aggiorna il size
			}
		}
	}
