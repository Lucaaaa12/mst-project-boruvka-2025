...
public class QuickFind<D> implements UnionFind<D> {
	...
	public void union(UnionFindNode<D> node1, UnionFindNode<D> node2) {
        if (find(node1) != find(node2)){
            ((QuickFindNode<D>)node1).root.tail.next = (QuickFindNode<D>)node2; //collega le liste di fratelli
            ((QuickFindNode<D>)node1).root.tail = ((QuickFindNode<D>)node2).root.tail; //aggiorna campo tail
            while (node2 != null){
                ((QuickFindNode<D>)node2).root = ((QuickFindNode<D>)node1).root;
                node2 = ((QuickFindNode<D>)node2).next;
            }
        }
    }
